 ...
 */
export function herdr<T = unknown>(args: string[], opts: HerdrOpts = {}): Promise<Result<T>> {
  ...
  return new Promise<Result<T>>((resolve) => {
    ...
    const timer = setTimeout(() => {
      ...
      finish(err("TIMEOUT", `herdr ${args.join(" ")} timed out after ${timeoutMs}ms`));
    }, timeoutMs);
    ...
    child.on("error", (e) => {
      ...
      if (code === "ENOENT") {
        finish(err("HERDR_UNAVAILABLE", "herdr binary not found on PATH (set HERDR_BIN)"));
      } else {
...
function err(code: HerdrErrorCode, message: string, details?: unknown): Result<never> {
