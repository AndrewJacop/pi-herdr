...
const start = await herdr(["agent", "start", "bootdiag", "--no-focus", "--", "cmd", "/c", "pi"], {
  timeoutMs: 20_000,
});
...
const vis = await herdr(["agent", "read", pane, "--source", "visible", "--lines", "12", "--format", "text"], {
  timeoutMs: 10_000,
  textOk: true,
});
...
if (booted) {
  ...
  const rd = await herdr(["agent", "read", pane, "--source", "recent", "--lines", "30", "--format", "text"], {
    timeoutMs: 10_000,
    textOk: true,
  });
