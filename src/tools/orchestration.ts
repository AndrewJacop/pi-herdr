...
const agentFields = {
  name: Type.Optional(
    Type.String({ description: "Agent pane name (must be unique). Default: agent-<timestamp>." }),
  ),
  agent: Type.Optional(
    StringEnum(AGENT_PRESETS, {
      description: "Built-in preset to launch (default 'pi'). Use 'custom' with an explicit argv.",
    }),
  ),
  argv: Type.Optional(
    Type.Array(Type.String(), {
      description: "Explicit launch argv; overrides the preset (required when agent='custom').",
    }),
  ),
...
function fail(r: Err): ToolReturn {
  return {
    content: [{ type: "text", text: `Error (${r.error.code}): ${r.error.message}` }],
...
async function resolvePaneId(target: string, signal?: AbortSignal): Promise<Result<string>> {
  const r = await herdr<unknown>(["agent", "get", target], { timeoutMs: 10_000, signal });
  if (!r.ok) return r as Result<string>;
  const a = (r.data as { agent?: Record<string, unknown> })?.agent ?? (r.data as Record<string, unknown>);
  const pid = (a?.pane_id as string) ?? (a?.paneId as string);
  if (!pid) {
    return {
      ok: false,
      error: { code: "NOT_FOUND", message: `No pane found for target "${target}"`, details: r.data },
    };
  }
...
export function registerOrchestration(pi: ExtensionAPI): void {
  ...
  pi.registerTool({
    ...
    parameters: Type.Object({
      ...agentFields,
      split: Type.Optional(
        StringEnum(["right", "down"] as const, { description: "Split direction relative to the current pane." }),
      ),
      tabId: Type.Optional(Type.String({ description: "Target tab id, e.g. 'w1:t1'." })),
      workspaceId: Type.Optional(Type.String({ description: "Target workspace id, e.g. 'w1'." })),
      env: Type.Optional(
        Type.Record(Type.String(), Type.String(), { description: "Extra env vars (KEY=VALUE) for the agent." }),
      ),
      focus: Type.Optional(Type.Boolean({ description: "Focus the new pane (default false)." })),
    }),
    async execute(_id, p, signal) {
      ...
      if (p.env) for (const [k, v] of Object.entries(p.env)) args.push("--env", `${k}=${v}`);
      args.push(p.focus ? "--focus" : "--no-focus");
      args.push("--", ...spec.data);

      const r = await herdr<{ agent?: Record<string, unknown> }>(args, { timeoutMs: 20_000, signal });
  ...
  pi.registerTool({
    ...
    promptGuidelines: ["Use herdr_send_prompt to send a prompt to an agent pane, then herdr_wait_agent + herdr_read_agent to get the reply."],
    parameters: Type.Object({
      target: Type.String({ description: "Pane id (w1:p3), agent name, or label." }),
    ...
    async execute(_id, p, signal) {
      ...
      const sendR = await herdr(["agent", "send", p.target, p.text], { timeoutMs: 15_000, signal });
      if (!sendR.ok) return fail(sendR);
      let submitted = false;
      if (p.submit !== false) {
        const enterR = await herdr(["pane", "send-keys", pid.data, "Enter"], { timeoutMs: 15_000, signal });
  ...
  pi.registerTool({
    ...
    promptGuidelines: ["Use herdr_read_agent to fetch an agent's response after herdr_wait_agent reports idle."],
    parameters: Type.Object({
      ...
      lines: Type.Optional(Type.Integer({ description: "Max lines to read (default 50)." })),
      format: Type.Optional(
        StringEnum(["text", "ansi"] as const, { description: "Output format (default 'text')." }),
      ),
    }),
    async execute(_id, p, signal) {
      ...
      const r = await herdr<unknown>(
        ["agent", "read", p.target, "--source", source, "--lines", String(lines), "--format", format],
      ...
      return okText(text || "(no output)", { paneId: p.target, text, truncated });
    },
  });
  ...
  pi.registerTool({
    ...
    promptGuidelines: ["Use herdr_wait_agent to block until an agent finishes a turn (status idle), then read its output."],
    parameters: Type.Object({
      target: Type.String({ description: "Pane id, agent name, or label." }),
      status: StringEnum(["idle", "working", "blocked", "done", "unknown"] as const, {
    ...
    async execute(_id, p, signal) {
      ...
      const r = await herdr<unknown>(
        ["wait", "agent-status", p.target, "--status", p.status, "--timeout", String(timeoutMs)],
  ...
  pi.registerTool({
    ...
    promptGuidelines: ["Use herdr_list_agents to see what agent panes exist and their idle/working status."],
    parameters: Type.Object({}),
    async execute(_id, _p, signal) {
      const r = await herdr<{ agents?: Record<string, unknown>[] }>(["agent", "list"], {
        timeoutMs: 10_000,
        signal,
      });
      if (!r.ok) return fail(r);
      ...
      return okText(
        agents.length
          ? `${agents.length} agent(s):\n` +
            agents
              .map((a) => `- ${a.paneId ?? "?"} [${a.agentStatus ?? "?"}] ${a.name ?? ""} (${a.agent ?? "?"})`)
              .join("\n")
          : "No agents running.",
        { agents },
      );
  ...
  pi.registerTool({
    ...
    async execute(_id, p, signal) {
      const r = await herdr<{ agent?: Record<string, unknown> }>(["agent", "get", p.target], {
        timeoutMs: 10_000,
        signal,
      });
      if (!r.ok) return fail(r);
  ...
  pi.registerTool({
    ...
    promptGuidelines: ["Use herdr_stop_agent to close an agent pane; it terminates that agent process."],
    parameters: Type.Object({
      target: Type.String({ description: "Pane id, agent name, or label." }),
    }),
    async execute(_id, p, signal) {
      const pid = await resolvePaneId(p.target, signal);
      if (!pid.ok) return fail(pid);
      const r = await herdr(["pane", "close", pid.data], { timeoutMs: 10_000, signal });
      if (!r.ok) return fail(r);
      return okText(`Closed pane ${pid.data} ("${p.target}").`, { paneId: pid.data, stopped: true });
    },
  });
  ...
  pi.registerTool({
    ...
    parameters: Type.Object({
      target: Type.String({ description: "Pane id, agent name, or label." }),
      name: Type.Optional(
        Type.String({ description: "New name; omit or set empty to clear the name." }),
      ),
    }),
    async execute(_id, p, signal) {
      const newName = p.name && p.name.length ? p.name : null;
      const r = await herdr(["agent", "rename", p.target, ...(newName ? [newName] : ["--clear"])], {
        timeoutMs: 10_000,
        signal,
      });
      if (!r.ok) return fail(r);
  ...
  pi.registerTool({
    ...
    async execute(_id, p, signal) {
      const r = await herdr(["agent", "focus", p.target], { timeoutMs: 10_000, signal });
      if (!r.ok) return fail(r);
      return okText(`Focused pane "${p.target}".`, { paneId: p.target, focused: true });
    },
  });
  ...
  pi.registerTool({
    ...
    description: "Get a natural-language explanation of what an agent pane is/does.",
    ...
    async execute(_id, p, signal) {
      ...
      return okText(explanation || "(no explanation)", { target: p.target, explanation });
    },
  });
  ...
  pi.registerTool({
    ...
    promptSnippet: "One-shot delegate: spawn an agent, send a prompt, wait, return its reply",
    promptGuidelines: [
      "Use herdr_delegate for one-shot delegation: it spawns an agent, sends the prompt, waits for idle, and returns the response.",
    ],
    parameters: Type.Object({
      ...agentFields,
      prompt: Type.String({ description: "Prompt to send to the spawned agent." }),
      timeoutMs: Type.Optional(
        Type.Integer({ description: "Overall budget in ms (default 120000)." }),
      ),
      closeOnSuccess: Type.Optional(
        Type.Boolean({ description: "Close the pane after a successful response (default false, keep alive)." }),
      ),
    }),
    async execute(_id, p, signal) {
      ...
      const partial = (message: string, extra: Record<string, unknown>, isError = true): ToolReturn => ({
      ...
      const startR = await herdr<{ agent?: Record<string, unknown> }>(startArgs, { timeoutMs: 20_000, signal });
      if (!startR.ok) return fail(startR);
      const paneId =
        ((startR.data?.agent as { pane_id?: string })?.pane_id ??
          (startR.data as { pane_id?: string })?.pane_id) ?? null;
      if (!paneId) {
      ...
      const readR = await herdr<unknown>(
        ["agent", "read", paneId, "--source", "recent", "--lines", "50", "--format", "text"],
        { timeoutMs: 15_000, signal, textOk: true },
      );
