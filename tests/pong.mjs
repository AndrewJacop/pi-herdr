...
try {
  ...
  const start = await herdr(["agent", "start", "pi-pong", "--no-focus", "--", "cmd", "/c", "pi"], {
    timeoutMs: 20_000,
  });
  ...
  const boot = await herdr(["wait", "agent-status", PANE.id, "--status", "idle", "--timeout", "60000"], {
    timeoutMs: 70_000,
  });
  check(boot.ok, `boot -> idle (code=${boot.error?.code})`);

  console.log("\n[pong] 3. send prompt + Enter");
  const send = await herdr(["agent", "send", PANE.id, "Reply with exactly one word: pong"], {
    timeoutMs: 15_000,
  });
  check(send.ok, `send ok`);
  const enter = await herdr(["pane", "send-keys", PANE.id, "Enter"], { timeoutMs: 15_000 });
  check(enter.ok, `enter ok`);

  console.log("\n[pong] 4. trace the turn: working -> idle");
  await herdr(["wait", "agent-status", PANE.id, "--status", "working", "--timeout", "30000"], {
    timeoutMs: 35_000,
  });
  const idle = await herdr(["wait", "agent-status", PANE.id, "--status", "idle", "--timeout", "120000"], {
    timeoutMs: 130_000,
  });
  ...
  const read = await herdr(
    ["agent", "read", PANE.id, "--source", "recent", "--lines", "40", "--format", "text"],
