 ...
 * source has to be an inlined string: the worker is plain CommonJS JavaScript
 * and never sees the TypeScript pipeline.
 *
 ...
 * second run and the replay silently diverges. (The journal itself is issue
 * 13's port; the jail is what makes it sound, so it lands here in 12.)
 */
 ...
 *
 * Module-scope, not local to main(). Held for the structured-output path when
 * issue 14 lands it; today only main()'s meta/args materialisation uses it, so
 * the script sees objects whose prototype is its own.
 */
let realmParse = null;
 ...
 *
 * pi-herdr trim: \`off\` added — the list must match the \`--thinking\` levels
 * herdr's routing chain enforces (src/launchplan.ts THINKING_LEVELS).
 */
 ...
 * which is what makes it share the run's agent counter, abort signal and
 * budget without any of them being plumbed anywhere. What it must NOT share
 * is ambient phase state, so that lives here and the child's globals are
 * closures over its own scope.
 */
 ...
 * it shares the run's agent counter, abort signal and budget without any of
 * them being passed anywhere: there is only ever one of each. What it does not
 * share is ambient phase state, which lives on the scope.
 *
 * One level only. The child's \`workflow\` is present and throws rather than
 * absent, so the error names the limit instead of reading
 * \`workflow is not defined\`.
 *
 * pi-herdr: resolution (saved names, \`scriptPath\`) is issue 13's port — until
 * then the host carries no \`loadWorkflow\`, and the runtime refuses the call
 * fatally rather than silently running nothing.
 */
