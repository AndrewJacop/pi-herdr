...
> **SUPERSEDED 2026-09-19 by the v0.6 map** (`../map.md`) and its tickets (`../tickets/`).
> The v0.6 re-spec (grilled against `0xRichardH/pi-herdr-subagents` + tintinweb's
> workflow system) replaced the result substrate (pane-tail → session files), the
> delivery model (pull → hybrid push), the status vocabulary (6 → 10 projected states),
> reversed the workflows out-of-scope ruling, cut the surface 43 → 12, and set a hard
> herdr ≥0.9.0 floor. These tickets remain the provenance of every decision that
> survived; read them as history, not spec.
