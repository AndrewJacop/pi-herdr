...
const visibleTail = async (pane, n = 6) => {
	const r = await herdr(["agent", "read", pane, "--source", "visible", "--lines", String(n), "--format", "text"], { timeoutMs: 8_000, textOk: true });
	if (!r.ok) return "";
	const t = r.data?.read?.text ?? r.data?.text ?? (typeof r.data === "string" ? r.data : JSON.stringify(r.data));
	return t.split(/\r?\n/).slice(-n).join("\n");
};

let pass = 0, fail = 0;
const check = (c, m) => { pass += c?1:0; fail += c?0:1; console.log((c?"  ✓ ":"  ✗ ")+m); };

// PLAIN pi — NO -e. Relies entirely on the global install.
const start = await herdr(["agent", "start", "global-probe", "--no-focus", "--", "cmd", "/c", "pi"], { timeoutMs: 20_000 });
...
try {
	...
	let booted = false;
	for (let i = 0; i < 60; i++) {
		if (await statusOf(pane) === "idle") { booted = true; break; }
	...
	check(loaded, `global pi-herdr loaded (footer shows herdr:)\n      footer: ${footer.replace(/\n/g, " | ").slice(-160)}`);

	console.log("--- send prompt + enter ---");
	await herdr(["agent", "send", pane, "Reply with exactly one word: pong"], { timeoutMs: 15_000 });
	await herdr(["pane", "send-keys", pane, "Enter"], { timeoutMs: 15_000 });

	// (b) self-report: working -> done, reliably (not stuck on working).
	let sawWorking = false, reachedDone = false;
	for (let i = 0; i < 150; i++) {
		const s = await statusOf(pane);
		if (s === "working") sawWorking = true;
		if ((s === "done" || s === "idle") && sawWorking) { reachedDone = true; console.log(`    reached ${s} after working (t=${i*2}s)`); break; }
